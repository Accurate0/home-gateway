...
**Zigbee model profiles (`config/zigbee_models.yaml`):** zigbee payloads are *not* parsed by per-model Rust structs — a device declares `model: <slug>` in its `config/devices/<type>.yaml` file and the slug's profile says which payload field feeds each role. A role block is a list of field names (`smart_switch: [state, voltage, power, current, energy]`) since zigbee2mqtt already standardizes property names; use a map (`{ power: active_power }`) only when a model deviates. The free-form `metrics:` block is the escape hatch for fields with no column of their own — each entry is `{ key, type }` and lands in the generic `device_metric` sink (`int`/`float` → `value`, `bool`/`string` → `text_value`), so exposing a new field is pure YAML. Everything is validated at startup: a zigbee device without a `model:`, an unknown slug, or a role the profile doesn't map is a hard error, and an unregistered address warns at runtime.

**When adding a new zigbee device:** add its address + `model:` to the matching `config/devices/<type>.yaml` (a new type file also goes in `devices/index.yaml` and `config/kustomization.yaml`), and add the profile to `config/zigbee_models.yaml` if that model isn't already there. No code change.

**When adding a new ESPHome environment sensor/device:** map metrics to object_ids directly in `config/devices/environment.yaml` — the environment kind's `entities:` is a required `metric: object_id` map (metrics: `temperature`/`humidity`/`pressure`/`lux`/`uv_index`/`pm25`/`voc_index`, the `Metric` enum in `src/settings/devices/environment.rs`, shared with the zigbee `environment:` block). No code change is needed for environment sensors. An esphome environment device with an empty `entities:` map is rejected at load. Non-environment esphome device kinds (presence, plant) still route by config entity lists.
