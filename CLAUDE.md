...
**When adding a new ESPHome environment sensor/device:** map metrics to object_ids directly in `config/devices.yaml` — the environment kind's `entities:` is a required `metric: object_id` map (metrics: `temperature`/`humidity`/`pressure`/`lux`/`uv_index`/`pm25`/`voc_index`, the `Metric` enum in `src/settings/environment.rs`, shared with the zigbee `environment:` block). No code change is needed for environment sensors. An esphome environment device with an empty `entities:` map is rejected at load. Non-environment esphome device kinds (presence, plant) still route by config entity lists.
