...
**When adding a new ESPHome sensor/device:** its object_ids must be known to the code — add object_id constants in `src/esphome/mod.rs` (and to `TEMPERATURE_SENSOR_OBJECT_IDS` for environment defaults), handle them in the relevant device actor's `Entity::Esphome` match, then register the device in `config/devices.yaml`.
