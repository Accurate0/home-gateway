...
module piPinholes() {
    let (
        ...
        pin_hole_locations = [
            [pin_holes_x + 76, pin_holes_y - 10],
            [pin_holes_x + width + 76, pin_holes_y - 10],
            [pin_holes_x + 76, pin_holes_y + height - 10],
            [pin_holes_x + width + 76, pin_holes_y + height - 10]
        ];
        for (loc = pin_hole_locations) {
            translate([loc[0], loc[1], -0.11])
            cylinder(d = 2.5, h = 6.00);
            translate([loc[0], loc[1], case_depth - pi_pinholes_spacer_height - 0.11])
            cylinder(d = pi_pinholes_spacer, h = 4.00);
        }
    }
}
