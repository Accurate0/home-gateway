...
esp_err_t initialSpi(void) {
  ...
  spi_bus_config_t spiBusConfig = {.data0_io_num = SPI_Data0,
...
void initialGpio(void) {
  ...
  ioConfig.pin_bit_mask = ((1ULL << EPD_RST) | (1ULL << SPI_CS0) |
...
esp_err_t spiTransmitData(unsigned char *dataBuffer, unsigned long dataLength) {
  ...
  while (dataLength >= SPI_MAX_BUFFER_SIZE) {
    ...
    status = spi_device_transmit(spi, &trans_ext.base);
  ...
  if (dataLength > 0) {
    ...
    // The trans_ext.base.length unit is bit, so the dataLength must be
    // multiplied by 8.
    trans_ext.base.tx_buffer = dataBuffer;
    trans_ext.base.flags = SPI_TRANS_VARIABLE_CMD;
    status = spi_device_transmit(spi, &trans_ext.base);
...
esp_err_t spiTransmitLargeData(unsigned char commandBuf,
  ...
  if (dataLength > 0) {
    if (firstPacket) {
      ...
      // The trans.length unit is bit, so the dataLength must be multiplied
      // by 8.
      trans.tx_buffer = dataBuffer;
      trans.rx_buffer = NULL;
      status = spi_device_transmit(spi, &trans);
...
esp_err_t spiTransmit(unsigned char commandBuf, unsigned char *dataBuffer,
  ...
  if (dataLength < SPI_MAX_BUFFER_SIZE) {
    ...
    status = -1; // The dataLength is over the SPI_MAX_BUFFER_SIZE

  return status;
}

esp_err_t spiReceive(unsigned char commandBuf, unsigned char *dataBuffer,
                     unsigned int dataLength) {
  esp_err_t status = 0;
  ...
  if (dataLength < SPI_MAX_BUFFER_SIZE) {
    ...
    // The trans.rxlength unit is bit, so the dataLength must be multiplied
    // by 8.
