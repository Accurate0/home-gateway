...
inline void draw_bottom_menu(
    ...
    if (left && left[0] == '^' && left[1] == '\0') {
        // triangle pointing up; 6x6 px centred at (23, 59)
        int cx = 20, cy = 59;
        it->line(cx + 2, cy - 3, cx + 3, cy - 3);
        it->line(cx + 1, cy - 1, cx + 4, cy - 1);
        it->line(cx + 1, cy,     cx + 4, cy);
        it->line(cx,     cy + 2, cx + 5, cy + 2);
        it->line(cx,     cy + 3, cx + 5, cy + 3);
    } else if (left && left[0] == '<' && left[1] == '\0') {
    ...
    if (right && right[0] == 'v' && right[1] == '\0') {
        // triangle pointing down; 6x6 px centred at (105, 59)
        int cx = 102, cy = 59;
        it->line(cx,     cy - 3, cx + 5, cy - 3);
        it->line(cx,     cy - 2, cx + 5, cy - 2);
        it->line(cx + 1, cy,     cx + 4, cy);
        it->line(cx + 1, cy + 1, cx + 4, cy + 1);
        it->line(cx + 2, cy + 3, cx + 3, cy + 3);
    } else if (right && right[0] == '>' && right[1] == '\0') {
