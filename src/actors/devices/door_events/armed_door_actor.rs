    ...
    pub fn trigger_action(&self, ieee_addr: &IEEEAddress) {
        if let Some(settings) = self.shared_actor_state.devices.door(ieee_addr) {
            ...
            notify(
                &settings.notify,
                Notification::new(
                    NotificationSource::DoorLeftOpen,
                    message,
                    NotifyCategory::Door,
                    format!("door:{ieee_addr}"),
                ),
            );
        }
    }
...
impl Actor for ArmedDoor {
    ...
    #[tracing::instrument(parent = None, name = "actor.armed_door", skip(self, myself, message, state), level = Level::TRACE)]
    async fn handle(
        &self,
        ...
        match event {
            DoorEventsType::Opened => {
                ...
                if let Some(value) = self.shared_actor_state.devices.door(&ieee_addr)
