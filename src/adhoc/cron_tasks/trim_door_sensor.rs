...
pub struct TrimDoorSensor;

#[async_trait::async_trait]
impl AdhocCronTask for TrimDoorSensor {
    fn name(&self) -> &'static str {
        "trim_door_sensor"
    }

    fn schedule(&self) -> CronSchedule {
        CronSchedule::parse("5 3 * * *").expect("valid cron")
    }
    ...
    async fn run(&self, ctx: &mut AdhocTaskContext<'_>) -> Result<u64, AdhocTaskError> {
        let chunks = sqlx::query_scalar!(
            "SELECT drop_chunks('door_sensor', older_than => now() - INTERVAL '1 year')::text"
        )
